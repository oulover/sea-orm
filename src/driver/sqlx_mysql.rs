            ...
            .map(|c| {
                (
                    ...
                    match c.type_info().name() {
                        "TINYINT(1)" | "BOOLEAN" => Value::Bool(row.try_get(c.ordinal()).ok()),
                        "TINYINT UNSIGNED" => Value::TinyUnsigned(row.try_get(c.ordinal()).ok()),
                        "SMALLINT UNSIGNED" => Value::SmallUnsigned(row.try_get(c.ordinal()).ok()),
                        "INT UNSIGNED" => Value::Unsigned(row.try_get(c.ordinal()).ok()),
                        "MEDIUMINT UNSIGNED" | "BIGINT UNSIGNED" => Value::BigUnsigned(row.try_get(c.ordinal()).ok()),
                        "TINYINT" => Value::TinyInt(row.try_get(c.ordinal()).ok()),
                        "SMALLINT" => Value::SmallInt(row.try_get(c.ordinal()).ok()),
                        "INT" => Value::Int(row.try_get(c.ordinal()).ok()),
                        "MEDIUMINT" | "BIGINT" => Value::BigInt(row.try_get(c.ordinal()).ok()),
                        "FLOAT" => Value::Float(row.try_get(c.ordinal()).ok()),
                        "DOUBLE" => Value::Double(row.try_get(c.ordinal()).ok()),

                        "BIT" | "BINARY" | "VARBINARY" | "TINYBLOB" | "BLOB" | "MEDIUMBLOB"
                        | "LONGBLOB" => Value::Bytes(row.try_get(c.ordinal()).ok()),

                        "CHAR" | "VARCHAR" | "TINYTEXT" | "TEXT" | "MEDIUMTEXT" | "LONGTEXT" => {
                            Value::String(row.try_get(c.ordinal()).ok())
                        }

                        #[cfg(feature = "with-chrono")]
                        "TIMESTAMP" => Value::ChronoDateTimeUtc(row.try_get(c.ordinal()).ok()),
                        #[cfg(all(feature = "with-time", not(feature = "with-chrono")))]
                        "TIMESTAMP" => Value::TimeDateTime(row.try_get(c.ordinal()).ok()),

                        #[cfg(feature = "with-chrono")]
                        "DATE" => Value::ChronoDate(row.try_get(c.ordinal()).ok()),
                        #[cfg(all(feature = "with-time", not(feature = "with-chrono")))]
                        "DATE" => Value::TimeDate(row.try_get(c.ordinal()).ok()),

                        #[cfg(feature = "with-chrono")]
                        "TIME" => Value::ChronoTime(row.try_get(c.ordinal()).ok()),
                        #[cfg(all(feature = "with-time", not(feature = "with-chrono")))]
                        "TIME" => Value::TimeTime(row.try_get(c.ordinal()).ok()),

                        #[cfg(feature = "with-chrono")]
                        "DATETIME" => Value::ChronoDateTime(row.try_get(c.ordinal()).ok()),
                        #[cfg(all(feature = "with-time", not(feature = "with-chrono")))]
                        "DATETIME" => Value::TimeDateTime(row.try_get(c.ordinal()).ok()),

                        #[cfg(feature = "with-chrono")]
                        "YEAR" => Value::ChronoDate(row.try_get(c.ordinal()).ok()),
                        #[cfg(all(feature = "with-time", not(feature = "with-chrono")))]
                        "YEAR" => Value::TimeDate(row.try_get(c.ordinal()).ok()),

                        "ENUM" | "SET" | "GEOMETRY" => Value::String(row.try_get(c.ordinal()).ok()),

                        #[cfg(feature = "with-bigdecimal")]
                        "DECIMAL" => Value::BigDecimal( row.try_get(c.ordinal()).ok().map(Box::new) ),
                        #[cfg(all(
                        ...
                        "JSON" => Value::Json(row.try_get(c.ordinal()).ok()),
